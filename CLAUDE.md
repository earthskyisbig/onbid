...
| 2026-09-02 | 결과 대시보드 추가 | scripts/build_dashboard.py, scripts/templates/dashboard.html, 오케스트레이터 Phase 5 | 사용자 요청("웹앱으로 결과 볼 수 있나"). _workspace 산출물(검색·검증·전략·시세·서류·보고서)을 단일 HTML 로 묶어 브라우저/아티팩트로 열람. 구·신 스키마 모두 표시, 회차 저감 스케줄 차트 포함 |
| 2026-09-02 | 공매 유성우 시각화 | scripts/snapshot_market.py, scripts/build_meteor.py, scripts/templates/meteor.html | 사용자 요청("재미있는 걸, 시각화 멋지게"). 압류재산 전체 물건·예약 회차를 전수 수집해 감정가 대비 비율 × 달력 캔버스에 계단 궤적으로 겹쳐 그림. 기준일 스크럽/재생, 예산 슬라이더로 "그날 예산 안에 들어오는 물건" 강조, 지역·용도 필터, 일별 입찰창 밀도 스트립, 물건 상세. 스냅샷은 시장 통계용으로도 재사용 가능 |
