...
## 5. 후속 제안 진행 결과 (같은 날 2차 세션)

§3 의 후속 항목 4건을 진행했다. 전부 실제 API 호출로 검증했고 테스트는 45 → 71개.

### 5.1 유찰횟수 추정 방식 — **기존 방식이 틀렸음을 실측으로 확인, 제거**
공고목록 API 전수(2,959행, 30페이지)를 받아 `pbancMngNo` 반복 횟수와 물건 실제 `usbdNft` 를 대조했다.

| 관찰 | 값 |
|------|-----|
| 고유 공고 | 359건 |
| 반복 횟수 분포 | 1회 37 / 2회 13 / 4회 14 / 6회 13 / 7회 12 / 9회 15 / **10회 253** |
| 2회 등장 공고(`202512-51010-00`)의 물건 usbdNft | 3~6회 (기존 추정: 1회) |
| 1회 등장·pbctNsq 027 공고의 물건 usbdNft | 3회 (기존 추정: 0회 → `--min-fails 2` 에서 탈락) |

결론: 반복 횟수 = **예약된 회차 수**. `--min-fails` 를 1단계에서 이 값으로 적용하면 실제 유찰 물건을 놓친다.
조치: `call_list_api` 에서 유찰 필터 제거, `summarize_pbanc_list` 로 고유 공고를 최신 공고일 순으로 압축만 하고, 유찰 필터는 단계2 `usbdNft` 와 단계3 상세값으로만 적용. 목록에 압수물(자전거 등) 비부동산이 섞이는 것도 확인 — 단계2 용도 사전필터가 걸러낸다.

### 5.2 비아파트 실거래 — `fetch_market_data.py --kind`
국토부 API 7종(오피스텔 매매/전월세, 연립다세대 매매/전월세, 단독다가구 매매, 토지, 상업업무용)이 이 키로 전부 승인돼 있음을 확인(2026-08 데이터 정상 수신). 종류별 엔드포인트·이름·면적·매칭 필드를 `KINDS` 표로 통합했고, 토지·상가는 ㎡당 중앙값과 대상 면적 환산가를 내며 감정가·최저가 비교를 환산가 기준으로 한다. 실호출: 은평 호안빌(rh) 매매 2건·전세 2건 수신, 광주 검천리(land) 1건·㎡당 83,141원, 은마(apt) 회귀 정상. `location-analysis` 스킬·에이전트의 인라인 토지 API 코드는 제거하고 스크립트 호출로 대체.

### 5.3 PDF 서류 분석 — `analyze_documents.py`
pdfplumber 로 텍스트·표 추출 → 정규식으로 감정평가액(후보 복수 시 총액=최댓값 + 후보 목록·문맥 보존)·기준시점·면적·공시지가·준공년도·구조·임차보증금·월차임·채권최고액·용도지역·건폐율/용적률 추출 → 리스크 키워드 15종 플래그(“유치권 없음” 같은 부정 문맥 구분) → 스캔본 판정 → `manual_review_needed` 생성. 출력은 document-analyzer 프로토콜 골격이며 `rights_analysis`·`eviction_risk`·`condition`·`unpaid_management_fee` 는 LLM 이 채우도록 비워 둔다. 검증: 헤드리스 Chrome 으로 만든 한글 감정평가서 PDF(`tests/fixtures/sample_appraisal.pdf`)에서 감정가 2.5억·기준시점·채권최고액 1.8억·제2종일반주거지역·건폐율 60%·유치권(부정) 등 전부 정확 추출.

### 5.4 취득세율 자동 산정 — `roi_calculator.py tax` / `--kind`
지방세법 §11·§13의2(2026-09 기준) 표를 `acquisition_tax_rate()` 로 코드화: 주택 6억 이하 1% / 6~9억 누진 `(가액×2/3억−3)%` / 9억 초과 3%, 85㎡ 초과 농특세, 조정 2주택·비조정 3주택 8%, 조정 3주택·비조정 4주택 12%, 비주택 4.6%, 농지 3.4%. `calc_auction` 에 `propertyKind`·`areaSqm`·`houseCount`·`adjustedArea` 를 주면 **시나리오별 입찰가에 맞춰** 세율을 각각 적용하고 `acquisitionTaxBasis` 에 근거·주의사항을 남긴다(6억 경계를 넘나드는 케이스를 테스트로 고정). 감면·일시적 2주택 예외는 미반영이며 notes 로 안내한다.

### 5.5 남은 제안
- 8/23 산출물 재생성(구 스키마) — 기록이므로 보류
- 네이버 호가 실호출 검증 — 봇 차단 위험, 보류
- `analyze_documents.py` OCR 경로(pytesseract) — 스캔본 판정만 하고 OCR 은 미구현
- 취득세 감면(생애최초·신축 미분양 등)·일시적 2주택 예외 — 사용자 상황 입력이 필요해 미구현
